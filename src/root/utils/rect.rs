...
///   user-space** coordinates. This is the same coordinate system the element's own `x`/`y`/`d`/`points` attributes
///   are authored in, unaffected by any transform applied to the element or its ancestors. It is also the
///   **object/fill** bounding box only: stroke width, markers, and clipping are not included (see
