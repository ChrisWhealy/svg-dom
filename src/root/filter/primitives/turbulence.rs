...
impl SvgFilter {
    ...
    ///
    /// `base_frequency` is a pre-built [`fmt::Arguments`] rather than a `&str`. This lets the two public callers pass
    ...
    /// collecting into an owned `String`. This is the same technique the private `gaussian_blur_args` helper uses
    /// for [`gaussian_blur`](Self::gaussian_blur)/[`gaussian_blur_xy`](Self::gaussian_blur_xy)'s `stdDeviation`.
    fn turbulence_args(
        &self,
    ...
    ///
    /// `num_octaves` layers that many progressively finer copies of the noise together (each doubling the frequency
    /// and halving the amplitude of the last). This is the standard "octave" technique from Perlin/fractal-noise
    /// literature generally. More octaves add finer detail, at a rendering cost that grows with `num_octaves`, up to
    /// a point (see below). `1` is a single smooth noise layer. `4`–`6` is a typical range for visibly detailed
    /// texture without excessive cost. Negative values are unsupported by SVG, hence the use of `u32` rather than a
    /// signed integer to prevent such values from being supplied through this API at all.
    ///
    ...
    /// which this becomes likely. Very large values may therefore have no additional visual effect, rather than
    /// continuing to add ever-finer, imperceptible detail. They also do not incur the rendering cost the "grows with
    /// `num_octaves`" description above would otherwise suggest.
    ///
    ...
    ///
    /// Use the returned [`SvgNode`]'s [`set_attr`](crate::SvgNode::set_attr) to set `result` when noise is consumed
    /// as `in2` rather than as a subsequent primitive's implicit `in` — its usual role. This happens, for example,
    /// when feeding a [`displacement_map`](Self::displacement_map) (the primitive this filter is almost always
    /// paired with).
    ///
    ...
    ///
    /// Also use `set_attr` for `stitchTiles` (not wrapped by a named parameter). `"stitch"` avoids a visible seam
    /// when the filter region is meant to tile seamlessly, at the cost of subtly adjusting `baseFrequency` to fit
    /// exactly.
    ///
    /// # Errors
    ///
    ...
    ///
    /// Unequal frequencies stretch the noise pattern along whichever axis has the lower frequency. This is useful
    /// for wood-grain or brushed-metal textures that should read as directional rather than isotropic, unlike
