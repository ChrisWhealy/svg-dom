...
impl SvgFilter {
    ...
    /// None of them are wrapped by a named parameter here, so the returned [`SvgNode`]'s
    ...
    /// default in the example below) is the *destination* rectangle the repetitions fill. Unlike an ordinary
    /// primitive, the spec defaults this destination rectangle to the whole filter region.
    ///
    ...
    /// inputs' subregions. The immediate input to `tile` therefore need not itself carry explicit
    /// `x`/`y`/`width`/`height`. It can instead inherit a narrower default from an earlier primitive in the chain
    /// that was itself explicitly narrowed. A generator with no referenced input, such as
    /// [`turbulence`](Self::turbulence) (used below) or [`image`](Self::image), has no earlier primitive to inherit
    /// from. Its own default subregion is therefore the whole filter region instead. This is why the example below
    /// narrows it explicitly rather than relying on inheritance.
    ///
    /// Whichever primitive in the chain actually needs narrowing, its effective subregion must be smaller than
    /// `tile`'s own output subregion. Otherwise there is nothing smaller than the destination rectangle to repeat,
    /// and `tile`'s output is indistinguishable from its unchanged input.
    ///
    ...
    /// and `stroke`. `tile` instead repeats a *filter-generated* tile as one step inside a filter graph. It can feed
