...
//!
//! Building a transform string with `format!` allocates a fresh heap `String` on every call.
//! That is harmless for one-off element creation, but transforms are among the most common *high-frequency* updates.
//! Dragging, sliders, knobs, pan/zoom, follow-the-pointer cursors, resize/selection handles — a handler for any of
//! these may fire dozens or hundreds of times per second.
//! Each of those `format!`s allocates a string, formats into it, hands it to `set_attr`, and drops it again.
//!
//! The browser still has to receive the new attribute value, so the DOM write cannot be avoided.
//! What *can* be avoided is the repeated Rust-side allocation.
//! These helpers take a caller-owned `&mut String` scratch buffer, clear it, write the new transform into it, and
//! reuse the same backing allocation on the next call. No new allocation happens unless the formatted text grows
...
//! callbacks*.
//! Event-driven drag/pointer handlers do not run through the animation loop, and so need their own reusable buffer.
//!
//! # Example
//!
//! ```rust,no_run
...
impl SvgNode {
    ...
    ///
    /// Sets the transform directly as a 2D affine matrix.
    /// See [`Matrix2D`] for what each field means and how they map onto the SVG function's arguments
    /// `a, b, c, d, e, f`.
    ///
    ...
    /// structurally cannot avoid.
    /// Their rounding error is multiplied by whatever coordinate the matrix uses for transforms.
    /// So the error scales with the geometry rather than staying fixed the way a rounded translation or
    /// rotation angle does:
    ///
    /// - When the rounding threshold is set to 3dp, a rotation's sine term rounds to `0.000` below about `0.0286°`
    ///   (`sin(0.0286°) ≈ 0.0005`).
    ///   So this rounding error can cause a very slow matrix-driven rotation to visibly stick and then jump, rather
    ///   than moving smoothly.
    ///
    /// - Each linear coefficient's rounding error — up to `0.0005` — is applied to whatever coordinate the matrix
    ///   acts upon.
    ///   So at large coordinates the resulting positional error grows with them.  For example at `x = y =
    ...
    /// element's local transform.
    /// Or use it for a slow/precise rotation, where `set_matrix`'s quantisation can round a small sine term in a way
    /// that introduces visual artefacts, or makes the rotation disappear entirely.
    ///
    /// If the coefficients come from [`ctm`](Self::ctm) or [`screen_ctm`](Self::screen_ctm) rather than a
    /// local-space source, read that method's doc comment first.
    /// Both are accumulated coordinate-conversion matrices, not generally this element's own local transform.
    /// So writing either back here unmodified would double-apply whatever ancestor — and, for `screen_ctm`, page —
    /// transform is already contributed.
    ///
    /// # Example
    ///
    /// ```rust,no_run
    ...
    ///
    /// Sets the transform directly as a 2D affine matrix, exactly as [`set_matrix`](Self::set_matrix) does.
    /// But formats all six [`Matrix2D`] fields with Rust's shortest round-trip `Display` representation, instead of
    /// `set_matrix`'s fixed three/one decimal places.
    ///
    ...
    /// Or use it for a slow/precise rotation, where `set_matrix`'s quantisation can round a small sine term to zero,
    /// thus introducing visual artefacts or making the rotation disappear entirely.
    ///
    ...
    /// coefficient precision is itself desirable.
    /// For example, to keep the attribute value short and easy to read in devtools.
    ///
    /// This is not the same as a general size advantage.
    /// `set_matrix_precise` can produce the *shorter* string for round-number coefficients, since `set_matrix`
    /// always writes three or one decimal places even for `0`.
    /// For example, an identity matrix is `matrix(1, 0, 0, 1, 0, 0)` via `set_matrix_precise`, but
