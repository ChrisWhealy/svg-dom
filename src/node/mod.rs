...
impl SvgNode {
    ...
    ///
    /// # Calling this from within one of the node's own handlers
    ///
    /// This is safe, including calling it on itself from inside the very handler that is currently running. Dropping a
    /// listener's underlying wasm-bindgen closure does not free its Rust environment immediately. The JS wrapper
    /// generated by wasm-bindgen reference-counts each invocation and defers the actual deallocation until that
    /// invocation's own call frame returns.
    ///
    /// So a handler that removes itself will keep running to completion on valid captured state, with the removal
    /// taking effect before the next dispatch. The handler's captured state is freed once the current call returns —
    /// never while it is still executing.
    ///
    /// # Example
    ///
    /// ```rust,no_run
    ...
    /// lineage, and calling it on the event whose handler is currently running is safe for the same reason: see
    /// [`clear_listeners`](Self::clear_listeners) for why.
    ///
    /// # Example
    ///
    /// ```rust,no_run
