...
//! transforms.
//!
//! Every method here crosses into the browser and may trigger synchronous style or layout calculation.
//! If the relevant geometry is already up to date the browser need not redo that work, but a caller has no way to
//! know which case applies from here.
//! None of these methods belong on a hot per-frame or per-pointer-move path without profiling for the actual
//! scene/browser combination in use.
//! The same caveat [`computed_text_length`](crate::SvgNode::computed_text_length) already carries for the same
//! reason.
//!
...
//! `get_point_at_length` takes an `f32` distance. This module widens every such value to `f64` on the way out —
//! and narrows [`point_at_length`](SvgNode::point_at_length)'s `distance` argument to `f32` on the way in — purely
//! for API uniformity.
//! It does not, and cannot, recover precision the browser never had.
//!
//! Narrowing that `distance` argument is not a plain `as f32` cast, either.
//! See [`point_at_length`](SvgNode::point_at_length)'s own doc comment for why an out-of-`f32`-range *finite* `f64`
//! is saturated, rather than allowed to become an actual `f32` infinity — which the browser's binding rejects
//! outright.
//!
//! [`SvgNode::bounding_client_rect`] is the one exception: it wraps `Element.getBoundingClientRect()`, a modern Web
//! API whose `DOMRect` has always used `double` (`f64`) fields.
//! No widening happens there, and no precision is lost at that boundary.
//!
//! In practice this rarely matters: `f32` still carries roughly 7 significant decimal digits, comfortably more than
//! any realistic SVG coordinate or pixel measurement needs.
//! It matters if a caller feeds one of these `f64` results into a calculation expecting genuine double precision —
//! for example, accumulating many small deltas over a long session.
//! The extra `f64` bits beyond `f32`'s precision are exactly zero, not "unknown."
//! So such a calculation degrades to `f32` accuracy, rather than gaining anything from the wider Rust type.
...
impl SvgNode {
    ...
    ///
    /// The underlying SVG DOM API uses 32-bit floating-point values.
    /// The returned coordinates are widened to `f64` for consistency with the rest of this crate's geometry types.
    ///
    ...
    /// `clip-path` applied to the element are **not** included.
    /// So the returned rect can be visibly smaller than everything actually painted on screen.
    /// This crate does not currently expose the options-taking overload (`getBBox(options)`) that would let a
    /// caller opt into the stroke/decorated/visible box instead.
    /// See `docs/design_notes/geometry.md` ("`bounding_box` wraps only the no-argument `getBBox`...") for why.
    ///
    ...
    /// and are non-rendering elements that do not implement `SVGGraphicsElement`.
    /// So this branch is part of this method's real, reachable contract, not a defensive-only check.
    ///
    ...
    ///
    /// The underlying SVG DOM API uses 32-bit floating-point values.
    /// The returned matrix components are widened to `f64` for consistency with the rest of this crate's geometry
    /// types.
    ///
    ...
    /// identity matrix.
    /// Otherwise the ancestor component gets double-applied. See `docs/design_notes/geometry.md`
    ...
    /// physical monitor/screen coordinates.
    /// Per the SVG specification it maps into the document viewport's
    ...
    ///   *different* operation.
    ///   It compares this element's [`ctm()`](Self::ctm) against its parent's `ctm()`, not
    ///   `screen_ctm()` at all.
    ///
    ...
    /// This includes `<text>`, `<tspan>`, `<textPath>`, `<use>`, `<image>`, `<g>`, and the root `<svg>`.
    /// Only `<rect>`, `<circle>`, `<ellipse>`, `<line>`, `<polyline>`, `<polygon>`, and `<path>` support it.
    ///
    /// The underlying SVG DOM API uses a 32-bit floating-point value.
    /// The returned length is widened to `f64` for consistency with the rest of this crate's geometry types.
    ///
    /// **Performance:** this call crosses into the browser and may trigger synchronous style or layout calculation.
    /// Profile before calling it inside a hot animation or pointer-move callback — the actual cost depends on the
    /// scene's complexity and whether the browser's geometry is already current.
    ...
    /// A **finite** `distance` outside that range does not error — it clamps to the path's start or end point, per
    /// the SVG specification.
    ///
    /// The underlying SVG DOM API uses 32-bit floating-point values on both sides of this call.
    /// The returned point's coordinates are widened to `f64` for consistency with the rest of this crate's geometry
    /// types, and `distance` itself is narrowed to `f32` before crossing into the browser.
    /// See the next section for why that narrowing needs more care than a plain cast.
    ///
    ...
    /// the browser.
    /// A plain `distance as f32` cast is not enough on its own.
    /// Rust saturates an out-of-`f32`-range finite `f64` — for example `f64::MAX` — to `f32::INFINITY`.
    /// Unlike an ordinary out-of-range *finite* distance, the browser's binding rejects an actually-infinite
    /// argument outright instead of clamping it.
    /// This is confirmed empirically in Chromium: passing `Infinity` throws `TypeError: ... non-finite`, while a
    /// large but still-finite `f32`-representable distance (e.g. `1e30`) clamps to the path end exactly as
    /// documented.
    /// To keep the clamping behaviour intuitive across the full `f64` domain, an out-of-`f32`-range *finite*
    /// `distance` is saturated to `f32::MIN`/`f32::MAX`, rather than being allowed to become an actual infinity.
    /// That then clamps to the path's start/end the same as any other out-of-range finite value.
    ///
    /// **Performance:** this call crosses into the browser and may trigger synchronous style or layout calculation.
    /// Profile before calling it inside a hot animation or pointer-move callback — the actual cost depends on the
    /// scene's complexity and whether the browser's geometry is already current.
    ...
    pub fn point_at_length(&self, distance: f64) -> Result<Point, Error> {
        ...
        // clamping.
        // See this method's own doc comment for the empirical confirmation.
