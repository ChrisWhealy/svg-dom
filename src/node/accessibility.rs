...
impl SvgNode {
    ...
    /// it.
    /// As per the accessible-name-and-description computation algorithm, `aria-labelledby` and `aria-label` — when
    ...
    /// according to the SVG language-selection rules.
    /// See [`title`](Self::title) and this method's `# Scope` section for why that is not simply "the first one"
    /// once multilingual siblings are involved. In the common single-title case, that child supplies the accessible
    ...
    /// creating a new one.
    /// When there was no `<title>` child at all, the brand-new one is inserted as this element's first child.
    /// That makes it unambiguously the one [`title`](Self::title) finds next time.
    ///
    ...
    /// to be exposed to assistive technology as its own separate object in the accessibility tree.
    /// That is exactly the point for meaningful icons, controls, diagrams, and diagram components.
    /// But naming every individual decorative path or primitive produces a noisy, cumbersome accessibility tree
    /// that works against the users it is meant to help.
    ///
    /// Because [`set_title`](Self::set_title)/[`set_desc`](Self::set_desc) are generic on [`SvgNode`], they are
    /// callable on almost any element this crate hands back, which makes it easy to over-apply them.
    /// As a rule of thumb: attach `<title>`/`<desc>` to elements that are meaningful on their own — icons, controls,
    /// whole diagrams, or a `<g>` representing one logical idea.
    /// Leave purely decorative geometry — the individual paths/shapes that only exist to render a larger meaningful
    /// group — unnamed, so it is not individually exposed.
    ///
    /// A `<title>`/`<desc>` also does not, by itself, make an element interactive: it makes a graphic
    /// *describable*, not a control.
    /// If an icon is meant to be operable — clickable, focusable, activatable from the keyboard — that behaviour
    /// has to be built independently.
    /// A suitable `role`, a `tabindex`, and keyboard event handling — none of these are provided by either method.
    ///
    ...
    /// user agent selecting the most appropriate one via `lang`/`xml:lang`.
    /// This crate does not implement that selection.
    /// These methods make **no attempt to keep a `<title>` singular** on an element whose DOM they did not build
    /// from scratch: if this element already has more than one `<title>` child — for example, one attached from
    /// externally authored markup, or a multilingual set built by hand — `set_title` updates only the first one,
    /// `title()` reads only the first one, and `remove_title` removes only the first one.
    /// Every other `<title>` sibling is left completely untouched by any of these three methods.
    ///
    /// Build or manage multilingual `<title>`/`<desc>` sets through the underlying DOM directly, via
    /// [`as_element`](Self::as_element), or this crate's generic tree/attribute methods.
    /// This convenience API intentionally does not grow a `lang`-aware surface.
    /// That would be a distinct, future capability, rather than an extension of what these methods already promise.
    ///
    /// # Errors
    ///
    ...
    ///
    /// This method reads the *first* direct `<title>` child.
    /// It does not run that computation, consult ARIA attributes, or account for language-tagged sibling `<title>`s.
    /// See [`set_title`](Self::set_title)'s `# Scope` section for the full explanation of that limitation.
    ...
    /// See [`set_title`](Self::set_title)'s `# Scope` section — if this element has more than one `<title>`
    /// sibling, only the first is removed.
    ...
    ///
    /// Calling this again updates the *first* `<desc>` child's text, rather than always creating a new one.
    /// When there was no `<desc>` child at all, the brand-new one is inserted immediately after an existing
    /// `<title>` — or as the first child, if there is no `<title>` either.
    ///
    /// See [`desc`](Self::desc) to read the current child text back, and [`remove_desc`](Self::remove_desc) to remove
    /// it.
    /// This method has exactly the same first-direct-child scope, and the same non-goal of language-aware
    /// management, as [`set_title`](Self::set_title).
    /// See that method's `# Scope` section for the full explanation.
    ...
    /// `# Use judiciously` section.
    /// A `<desc>` on every decorative primitive is just as noisy for assistive technology as a `<title>` on every
    /// one.
    ///
    /// # Errors
    ///
    ...
    /// on this element, takes precedence over a `<desc>` child in accessible-description computation.
    ///
    /// This method reads the *first* direct `<desc>` child.
    /// It does not run that computation, consult ARIA attributes, or account for language-tagged sibling `<desc>`s.
    /// See [`set_title`](Self::set_title)'s `# Scope` section (which [`set_desc`](Self::set_desc) shares) for the
    /// full explanation of that limitation.
    ...
    fn set_accessible_child(&self, tag: &'static str, text: &str) -> Result<(), Error> {
        // SVG 2 requires authoring tools not to produce an empty or whitespace-only <title>/<desc>.
        // Either can expose an apparently nameless object to assistive technology — or, for <title>, suppress an
        // otherwise-usable accessible name derived from other content.
        // Reject outright rather than silently creating a blank element or reinterpreting this as a removal request.
        ...
        //
        // <desc> is inserted immediately after an existing <title> when there is one.
        // That matches the SVG specification's own examples, and keeps the two in the conventional order when both
        // are set, whichever order the caller calls set_title/set_desc in.
        // With no <title> present, <desc> instead becomes the first child itself, exactly as <title> would.
