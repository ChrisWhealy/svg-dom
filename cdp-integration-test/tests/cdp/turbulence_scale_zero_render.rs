//! `SvgFilter::displacement_map`'s `scale` argument at `0.0`, against real rendered pixels.
//!
//! The demo gallery's own turbulence panel (`demo/panels/panel-turbulence.html`) prominently states that scale 0
//! restores a perfect geometric circle.
//! `demo-app/src/browser_tests/paint/turbulence.rs` proves the DOM half of that claim: that the scale slider does
//! reach `scale="0"` on the real `feDisplacementMap` element.
//! It cannot prove the circle actually *renders* as a perfect circle at that value, since `wasm-bindgen-test`'s
//! WebDriver-run tests have no access to rasterised output.
//!
//! A structural test is satisfied by a `scale="0"` attribute sitting on a filter chain that renders however it
//! likes.
//!
//! This renders three circles built by the sibling `cdp-test-fixture` wasm crate:
...
//! offsets around each circle's own centre, not at shared absolute coordinates.
//! It samples eight points around each circle's own boundary, at matching angles, 3px inside and 3px outside the
//! nominal radius, then asserts:
//!
...
//!    This is the negative control: zero displacement should look unchanged.
//! 2. The reference and scale-sixty circles rasterise to a *materially different* pixel value at, conservatively,
//!    at least one sample.
//!    This is the positive control: a real, substantial displacement should look visibly different.
//!
//! Both checks matter together.
//! The first alone is a one-sided claim: it is equally consistent with a correctly working filter chain and with
//! a browser that silently ignores the filter (or fell back to unfiltered `SourceGraphic`), since either would
//! also rasterise like the reference.
//!
//! The second proves this fixture and sampling method can actually detect a real displacement in the first place,
//! so a pass on the first check means what it claims rather than reflecting an insensitive test.
//!
...
//! there, not on `scale` alone.
//! Below scale 6, `scale / 2` itself is under the 3px sample margin either check uses, and even a much larger
//! scale need not reach its own maximum at any particular sample.
//!
//! Scale 60 was chosen, and the threshold below calibrated, against what this sandbox's own headless Chrome
//! actually renders at that scale.
//! This is an observed property of this fixture, not an assumed one — see the positive control's own comment
//! further down for the measured numbers.
//!
//! Samples are not taken exactly on the mathematical radius.
//! That knife-edge pixel is roughly half-covered by design, so its exact rasterised value is unusually sensitive
//! to any small positional difference between the two circles.
//! This is the same reason `filter_blend_render.rs` samples its own corner pixel 2px inset from the shape's edge,
//! rather than exactly on it.
//! In this sandbox specifically, that turned out to matter for a second, more surprising reason too:
//! `cdp-test-fixture` pins `#turbulence-scale-zero`'s own filter region to exactly the circle's bounding box
//! (`set_x`, `set_y`, `set_width`, and `set_height`, all in `objectBoundingBox` units) rather than leaving it at
//! SVG's own default 10%-margin region.
//!
//! Left at that default, headless Chrome's software rasteriser (run using `--disable-gpu`, see this binary's own
//! `sandbox(false)` reasoning in `tests/cdp/main.rs`) composited the filtered circle back onto the page with a
//! real, several-pixel positional error, unrelated to `scale`.
//! This error was present even at `0.0`, and large enough on its own to fail this test's boundary samples
//! unpredictably from one run to the next.
//! See `cdp-test-fixture/src/lib.rs`'s own comment on this scenario for the full account of tracking down that
//! little gremlin.
//!
//! No attempt is made at broad screenshot testing across every slider position — a single identity test at scale
//! zero is enough to cover this specific, exact semantic claim without turning into a fragile visual regression
//! suite.
//!
//! # How the pixels are read
//!
//! Same technique as `filter_blend_render.rs`: serialise the fixture's `<svg>` to a `data:image/svg+xml` URL,
//! load it into an `Image`, draw it to an offscreen `<canvas>`, then read pixels back via `getImageData`.
//!
//! Since `Image` loading is asynchronous, this script must also be asynchronous, so it runs via
...
//! That wrapper hardcodes `returnByValue: false`, which only inlines primitive results, not the object this
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#[test]
fn turbulence_scale_zero_matches_reference_while_scale_sixty_visibly_differs() -> Result<(), String> {
    ...
    if reference_samples.len() != 16 {
        return Err("expected 8 inside + 8 outside samples for the reference circle".to_owned());
    }
    if scale_zero_samples.len() != 16 {
        return Err("expected 8 inside + 8 outside samples for the scale-zero circle".to_owned());
    }
    if scale_sixty_samples.len() != 16 {
        return Err("expected 8 inside + 8 outside samples for the scale-sixty circle".to_owned());
    }
    ...
    for (i, (reference, scale_zero)) in reference_samples.iter().zip(scale_zero_samples.iter()).enumerate() {
        ...
        if diff > ANTIALIASING_TOLERANCE {
            return Err(format!(
