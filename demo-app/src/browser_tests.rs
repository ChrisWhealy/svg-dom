...
//! (`demo_text_path`).
//! `structure` contributes the marker viewBox zoom slider (`demo_marker_view_box`) and the preserveAspectRatio
//! radio group (`demo_image`).
//! `paint` contributes several demos' own controls.
//! `demo_linear_gradient` contributes five sliders.
//! `demo_radial_gradient` contributes a slider, a radio group, and a slider pair.
//! `demo_filter` contributes four sliders: blur, dx, dy, and drop-shadow blur.
//! `demo_blend` contributes a BlendMode dropdown.
//! `demo_component_transfer` contributes three sliders: gamma exponent, discrete step count, and alpha slope.
//!
...
fn demo_component_transfer_sliders_update_gamma_discrete_and_alpha_independently() {
    ...
    super::paint::demo_component_transfer::demo().expect("demo_component_transfer::demo should build without error");
