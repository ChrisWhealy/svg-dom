...
* `desc`
* `ellipse`
* `filter` (with `feGaussianBlur`, `feOffset`, `feMerge`/`feMergeNode`, `feFlood`, `feComposite`, `feBlend`, `feDropShadow`, `feColorMatrix`, `feComponentTransfer`/`feFuncR`/`feFuncG`/`feFuncB`/`feFuncA`)
* `g`
* `image`
* `line`
