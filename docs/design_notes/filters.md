...
Copying `Spot` is not literally free (it is seven `f64`s plus an `Option<f64>`, not a single word), but it is allocation-free and cheap relative to the DOM operations each call site actually performs, and withholding `Copy` here would only make the enum harder to use (an explicit `.clone()` at every call site, including reusing one light description across a `diffuse_lighting`/`specular_lighting` pair, as the demo does) for no corresponding safety benefit — so it derives `Copy` the same way `Point` does.
