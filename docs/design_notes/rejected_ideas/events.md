...
One suggestion: have `EventListener` store the event name as an enum — `Click`, `PointerMove`, ... plus `Raw(&'static str)` — rather than a `&'static str`.
The grounds: an enum would be smaller than a fat string pointer, with `Drop` calling `event_name.as_str()`.

The premise upon which this idea is based is incorrect.

* **The enum is larger, not smaller.**<br>
  The `Raw(&'static str)` variant is mandatory: it backs the `on_event` escape hatch for arbitrary event names.
  So the enum must hold a `&'static str` and also distinguish it from the ~30 builtin named variants.
  On wasm32 a `&'static str` is 8 bytes (a 4-byte pointer plus a 4-byte length).
  The payload offers a single niche — the pointer is non-null, so one forbidden value — which can absorb only one unit variant for free.
  With the ~30 built-in event names, the layout falls back to an explicit discriminant, making the enum roughly 12 bytes.
...
* **The field is already a negligible part of `EventListener`.**<br>
  ...
  Trimming the event-name field, if that is even possible, would not achieve a meaningful reduction in total size.
  The field is already optimal at `&'static str` — the crate has already deliberately moved off `String`.

* **It adds standing maintenance for a negative payoff.**<br>
  The enum would have to enumerate every supported browser event name and stay in lockstep with the `on_*` helpers.
  It would also have to carry an `as_str()` mapping — all in exchange for making the struct bigger.

The recommendation sat behind a caveat: it is "only worth doing if listener-heavy scenes are expected."
But listener-heavy scenes are exactly where the larger per-listener struct cost would be greatest.

## Rewriting `ListenerStore::push` to push into the `Many` vector in place

`ListenerStore::push` replaces `self` with a non-allocating `Many(Vec::new())` placeholder, moves the old value out by value, and matches it exhaustively.
`One` upgrades to a two-element `Many`.
`Many` pushes the new listener and puts the vector back.

One suggestion: have the `Many` case push into the vector in place instead — matching `self` by reference, with a separate `mem::replace` + `unreachable!()` only on the `One` → `Many` upgrade path.
The goal was to avoid moving the whole `Vec` out and back.

This crate makes every effort to avoid generating a WASM binary that contains an `unreachable!()` call.
If that statement were ever reached, the WASM runtime would terminate the binary immediately and tear down its memory — effectively self-destructing.
...
* **The two properties are in tension: the current form is deliberately kept because it is panic-free.**<br>
  ...
  Matching that owned result can either handle *both* variants meaningfully — today's code, which is exhaustive and does not panic.
  *Or* it can pre-narrow `self` by reference, which then needs `unreachable!()` for the "impossible" arm.
  Rust's move rules do not allow both at once.

* **The saving is nil and the cost is real (if small).**<br>
  `Vec::new()` does not allocate.
  So the `Many` arm only moves the vector's 24-byte `(ptr, len, cap)` handle out and back.
  There is no extra heap allocation beyond the `push` itself, and `push` runs at listener-registration time, never on a hot path.
  Against that zero saving, `unreachable!()` adds the possibility of a panic and `#[track_caller]` location data to the **wasm binary**.
  That is a size concern this crate takes seriously — see [A size-optimised `[profile.release]` baked into the crate](performance.md#a-size-optimised-profilerelease-baked-into-the-crate).
  The optimiser *may* prove the arm dead and strip it, but that is not guaranteed.

The current code is exhaustive, panic-path-free, allocation-neutral, and documented as such at the call site.
The proposal is therefore a lateral-to-slightly-worse change to working code, and was not adopted.

## Deferred listener drops to make self-removal safe from within a handler

`clear_listeners` and `remove_listeners` are safe Rust APIs.
But their docs warn that calling them from inside one of the same node's handlers would free the currently-executing wasm-bindgen `Closure`.
That is undefined behaviour (UB) in the Rust abstract machine.
This recommendation proposed resolving that by deferring listener removal while a handler is dispatching.
It would use a depth counter and a side-store for closures queued for drop:
...
* **The UB is practically inert in the wasm32 execution model.**<br>
  ...
  In the Rust abstract machine this is formally UB.
  But in the concrete wasm32 execution environment there is no observable memory corruption, no torn state, and no crash.
  The warning documents a theoretical violation, not a practical hazard.

* **Deferred drops impose a permanent per-node memory cost.**<br>
  Adding `Cell<u32>` plus `RefCell<Vec<Box<ListenerStore>>>` to `SvgNodeInner` costs roughly 30 bytes on every node — depth counter, borrow flag, and `Vec` metadata.
  That is paid even though the self-removal pattern is vanishingly rare.
...
* **Tracking dispatch depth adds overhead on the hot path.**<br>
  ...
  Today `on_event` stores the raw user closure directly.
  Tracking depth would require wrapping each user closure in an outer bookkeeping closure.
  That adds an extra heap allocation per listener registration, and one extra increment/decrement/branch on every event fired.
  This crate deliberately avoids per-invocation overhead.
  The transform setters, `CachedAttr`, and the scratch-buffer APIs all exist specifically to reduce work done per event.
...
* **The motivating use case (a one-shot handler that removes itself) already has a correct, zero-cost solution.**<br>
  ...
  Combined with the browser-native `addEventListener` option `{ once: true }`, the closure is called exactly once.
  The memory is reclaimed by wasm-bindgen's own cleanup path after the call returns — never while the closure is still running.

  `on_event_once` was implemented using a `FnOnce` wrapped in a `FnMut` via `Option::take`, combined with `{ once: true }`.
  So the browser removes the DOM listener, and the user handler capture is freed after the first matching dispatch.
  A small listener shell — the `Closure<dyn FnMut(Event)>` wrapper — remains in the node's listener store until `clear_listeners`, `remove_listeners`, or node drop.
  That gives the same ownership guarantees as every other managed listener, without requiring a separate store design.
...
* **The remaining sub-cases are already safe and need no guards.**<br>
  ...
  Only the precise sub-case — a handler drops itself — is the footgun the docs warn against.
  `on_event_once` is the right API for that pattern.

The deferred-drops design would add memory and CPU overhead to every node and every event in the entire crate.
The primary motivating use case is already correctly served by `Closure::once`.

A dedicated `on_event_once` helper is the right follow-on addition, if the one-shot pattern proves common in practice.
That part of the conclusion still holds — see the correction below for why the premise changed.

### Correction (2026-09-24): the Undefined Behaviour (UB) premise was wrong

This analysis assumed `Closure::drop` frees the Rust environment immediately, with no re-dereference after the call returns.
On that assumption, self-removal was formally UB but practically inert.

The assumption is wrong.
The JS wrapper (`makeMutClosure`) generated by wasm-bindgen reference-counts each invocation.
It increments a count before calling the Rust closure, then decrements it in a `finally` block after the call returns.

Dropping the `Closure` from inside its own call only runs `_wbg_cb_unref`.
That decrements the count without reaching zero, since the enclosing invocation still holds one reference.
The actual Rust-side deallocation (`__wbindgen_destroy_closure`) fires from the `finally` block of the enclosing invocation, after the handler has already returned control to the trampoline.

So the closure's captured state is never freed while the handler runs.
This is confirmed against this crate's own generated JS glue (`makeMutClosure` in the wasm-pack build output), not just wasm-bindgen's source comments.
There is no abstract-machine UB here, not even a theoretical one — the deallocation is genuinely deferred by wasm-bindgen itself, at no extra cost to this crate.

[`clear_listeners`](../../../src/node/mod.rs) and [`remove_listeners`](../../../src/node/mod.rs) document the corrected behaviour.
`tests/svg_node/events.rs` has browser tests confirming a self-removing handler completes, is not re-invoked, and frees its captured state right after dispatch returns.

The conclusion not to build deferred-drop infrastructure still holds, but for a different reason.
wasm-bindgen already provides it, not because the theoretical UB was harmless to ignore.
The [next section](#listener-removal-has-documented-unsafe-lifecycle-caveats) analyses a follow-on proposal against this same wrong premise; its own conclusions on the handle-ownership dilemma are unaffected, since they hold independently of whether the UB premise is correct.
...
The `callback_ref` method has an 8-arm match with identical bodies: `closure.as_ref().unchecked_ref()`.
Every `on_*` helper constructs the matching variant.
...
The 35-line `EventClosure` enum and the `callback_ref` match would disappear.
`EventListener` would shrink by the discriminant — roughly 4 bytes per listener on wasm32.
...
* **It adds an extra `vtable` dispatch on every event fire.**<br>
  ...
  The cost of calling event handlers is already dominated by the JS/WASM boundary crossing.
  So this extra dispatch does not dominate either.
  However, adding overhead to the dispatch path conflicts with the crate's explicit design philosophy of keeping hot-path costs minimal.
...
* **The saving is marginal and the code change is lateral, not a reduction.**<br>
  ...
  In exchange, every `on_*` helper — and there are roughly 25–30 of them — gains a wrapper closure expression.
  `store_listener` also changes its parameter type.
  The net difference is roughly neutral in lines.
  The resulting registration sites are slightly harder to read, because the typed relationship between method name, event type, and closure is no longer encoded at the call-to-store boundary.
...
* **The WASM binary impact is speculative.**<br>
  ...
  The net binary size impact is unknown without measuring.
  The recommendation itself says "I would only keep it if the resulting wasm size ... looks good" — meaning the outcome is uncertain.
...
* **The per-listener struct size saving is negligible.**<br>
  On wasm32 a `Closure<dyn FnMut(T)>` is the same size regardless of `T`.
  Only the `vtable` pointer differs, and that is stored in the heap-allocated `Box<dyn FnMut>`, not in the `Closure` struct itself.

  The saved discriminant is ~4 bytes per `EventListener`.
  With `ListenerStore::One` that is a single listener.
  `ListenerStore::Many` allocates a `Vec` whose elements are individually ~4 bytes smaller.

  The saving is real but not significant enough to justify the change.

The current `EventClosure` enum is boilerplate.
It is nonetheless working, auditable boilerplate that encodes a clear structural invariant.
...
The claim was that this model "marks the listener as removed, detaches it from DOM, and defers dropping the closure until after the current dispatch completes."
That, the claim said, would make self-removal safe without the dispatch-depth counter.
...
The only mechanism that provides this is the dispatch-depth counter.
That is the same infrastructure rejected [above](#deferred-listener-drops-to-make-self-removal-safe-from-within-a-handler) for adding overhead to every event on every node.
...
**Handle drop is a no-op — the listener lives independently.**<br>
...
This adds no safety property over what `clear_listeners`/`remove_listeners` already provide, which — per the
[correction](#correction-2026-09-24-the-ub-premise-was-wrong) above — is already safe without a dispatch-depth
counter.

Neither option adds anything `clear_listeners`/`remove_listeners` do not already do.

### The underlying analysis from the deferred-drops rejection still applies

> **Correction (2026-09-24):** the first bullet below restates the wrong UB premise corrected
> [above](#correction-2026-09-24-the-ub-premise-was-wrong).
> Self-removal is safe because wasm-bindgen genuinely defers deallocation until the enclosing invocation returns, not
> merely because a use-after-free happens to leave no observable trace on wasm32.
> The other two bullets are unaffected.

- ~~The abstract-machine UB is practically inert in the wasm32 execution model.~~
  wasm-bindgen's own reference-counted closure wrapper defers the actual deallocation until the enclosing invocation
  returns, so there is no use-after-free to be inert about.
- The motivating use case (a handler that removes itself) is already correctly served by `on_event_once`.
  It uses `{ once: true }` and `Option::take`, so the browser removes the DOM listener and the user handler's captures are freed after the first dispatch — never while executing.
...
A handle that carries the identity of a specific listener would enable removal of just one listener, out of several registered for the same event type.
`remove_listeners("click")` cannot do this: it removes all listeners for the specified event type.
This is a new, incremental feature request, not a safety fix.

If specific-listener removal proves necessary in practice, it can be evaluated on its own merits.
It would need an API designed for that use case, not one framed as a correction to the existing safety model.
...
The web-sys extern type itself (the struct wrapping a `JsValue`) carries no code of its own.
It is an opaque handle that generates no WASM instructions.
...
Gating these modules behind optional features would remove only their Rust-level impl code, not any web-sys bindings.
DCE already removes that code if the types are never instantiated.
...
This crate's consistent position on speculative binary-size proposals is to decline until a measurement can show that a real reduction exists.
See [Performance & binary size](performance.md) for the other entries this has been applied to: the `ryu`/`itoa` crate, `path_fmt`/`text_fmt`, handle-light factories, and the error-path formatting machinery.
See also [flattening `EventClosure`](#flatten-eventclosure-by-simplifying-it-to-closuredyn-fnmutevent) above, within this category.
The same gate applies here.

## Optional delegated event handling for dense interactive scenes

An external review proposed a delegated listener API.
A single bubbling event handler attached to an `SvgRoot` or group would serve all matching descendants, reducing `N` per-node registrations down to one:
...
The handler fires for events originating from all descendants.
`event.target()` identifies the originating element.
...
The RFC's proposed `on_delegated_click` API adds nothing that the current API cannot express.
It would be a thin convenience wrapper around exactly the pattern shown above.
...
`SvgNode` is `Rc<SvgNodeInner>`, a reference-counted smart pointer to an opaque inner struct.
...
- use DOM-object identity keys, since `web_sys::Element` does not implement `Hash` or `Eq`. Any comparison would require `JsValue::loose_eq` or `Object.is()`.
...
It adds non-trivial runtime overhead on every node creation and destruction, solely to avoid calling `get_attribute` inside the closure.
...
For click events this is harmless.
But the RFC also mentions `pointermove`, keyboard events, and drag events as appropriate event types.
For `pointermove` at 60 Hz with an active scene, a JS call per event for selector evaluation reintroduces boundary crossings in the hot path.
That partially erases the registration savings the feature was designed to provide.
...
Introducing a parallel ownership model requires documentation carve-outs.
It also requires lifecycle caveats distinct from the existing listener-management guidance.
And it requires decisions about whether delegated registrations participate in `clear_listeners` and `remove_listeners`, whose semantics have already been carefully defined for the node-owned case.
