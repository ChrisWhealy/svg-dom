...
- `ctm()` / `screen_ctm()` — current transformation matrix, returned as the same role-named `Matrix2D` used by `set_matrix`/`set_matrix_precise`. `ctm()` accumulates every ancestor transform up to the nearest *viewport* ancestor; `screen_ctm()` continues all the way to the document viewport's CSS-pixel coordinates (despite its name, not physical screen/monitor coordinates), additionally carrying the root `<svg>`'s own position on the page. Both are **accumulated coordinate-conversion matrices, not generally this element's own local transform** — writing one straight back via `set_matrix`/`set_matrix_precise` is only correct when the parent-to-viewport transform is the identity matrix. Converting a *point* between viewport coordinates and this element's local coordinates (invert this element's own `screen_ctm()`) and recovering this element's own writable local *transform* (compare this element's `ctm()` against its parent's) are two different operations — see [`design_notes/geometry.md`](design_notes/geometry.md#ctmscreen_ctm-are-accumulated-matrices-not-generally-the-elements-own-local-transform) for both. Both `ctm()`/`screen_ctm()` return `None` if the element is not currently rendered.
