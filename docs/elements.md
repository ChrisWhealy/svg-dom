...
- `ctm()` / `screen_ctm()` — current transformation matrix, returned as the same role-named `Matrix2D` used by `set_matrix`/`set_matrix_precise`. `ctm()` accumulates every ancestor transform up to the nearest *viewport* ancestor; `screen_ctm()` continues all the way to screen space, additionally carrying the root `<svg>`'s own position on the page. Both are **accumulated coordinate-conversion matrices, not generally this element's own local transform** — writing one straight back via `set_matrix`/`set_matrix_precise` is only correct when the parent-to-viewport transform is the identity matrix; see [`design_notes.md`](design_notes.md#ctmscreen_ctm-are-accumulated-matrices-not-generally-the-elements-own-local-transform) for the general recovery formula. Both return `None` if the element is not currently rendered.
