...
The maximum displacement along either axis is `scale / 2` — 30px at scale 60 — but that is a ceiling, not a guarantee: the actual displacement at any one sampled point depends on the local turbulence channel value there, not on `scale` alone.
