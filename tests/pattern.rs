...
fn should_reject_invalid_view_box() -> Result<(), String> {
    ...
    check(
        matches!(pat.set_view_box(0.0, 0.0, f64::INFINITY, 100.0), Err(Error::InvalidViewBox(_))),
        "expected InvalidViewBox for a +infinity component",
    )?;
    check(
        matches!(
            pat.set_view_box(0.0, 0.0, f64::NEG_INFINITY, 100.0),
...
fn should_write_fractional_view_box_values() -> Result<(), String> {
    let svg = make_svg("pat-viewbox-fractional");
    let defs = svg.defs().map_err(|e| e.to_string())?;
    let pat = defs.pattern("pvb-fractional").map_err(|e| e.to_string())?;
    pat.set_view_box(-3.25, 12.5, 100.75, 50.125).map_err(|e| e.to_string())?;
    check_eq(
        pat.as_element().get_attribute("viewBox"),
...
fn should_accept_zero_width_or_height_alone() -> Result<(), String> {
    let svg = make_svg("pat-viewbox-zero-alone");
    let defs = svg.defs().map_err(|e| e.to_string())?;
    let pat = defs.pattern("pvb-zero-alone").map_err(|e| e.to_string())?;
    pat.set_view_box(0.0, 0.0, 0.0, 100.0).map_err(|e| e.to_string())?;
    check_eq(pat.as_element().get_attribute("viewBox"), Some("0 0 0 100".into()))?;
    pat.set_view_box(0.0, 0.0, 100.0, 0.0).map_err(|e| e.to_string())?;
    check_eq(pat.as_element().get_attribute("viewBox"), Some("0 0 100 0".into()))
}
...
fn should_preserve_previous_view_box_after_failed_validation() -> Result<(), String> {
    let svg = make_svg("pat-viewbox-preserve");
    let defs = svg.defs().map_err(|e| e.to_string())?;
    let pat = defs.pattern("pvb-preserve").map_err(|e| e.to_string())?;
    pat.set_view_box(1.0, 2.0, 300.0, 400.0).map_err(|e| e.to_string())?;
    check(
        matches!(pat.set_view_box(0.0, 0.0, -100.0, 100.0), Err(Error::InvalidViewBox(_))),
        "expected the second, invalid call to fail",
    )?;
    check_eq(pat.as_element().get_attribute("viewBox"), Some("1 2 300 400".into()))
}
