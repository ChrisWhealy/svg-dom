...
fn should_write_text_content_via_set_text_fmt() -> Result<(), String> {
    ...
    let (x, y) = (12.0, 34.0);
    label.set_text_fmt(&mut buf, format_args!("box: {x:.0}, {y:.0}")).map_err(|e| e.to_string())?;
...
fn should_reuse_buffer_across_set_text_fmt_calls() -> Result<(), String> {
    let label = make_svg("node-set-text-fmt-reuse")
        ...
        .map_err(|e| e.to_string())?;
    let mut buf = String::new();
    label.set_text_fmt(&mut buf, format_args!("box: {}, {}", 1, 2)).map_err(|e| e.to_string())?;
    label.set_text_fmt(&mut buf, format_args!("box: {}, {}", 30, 40)).map_err(|e| e.to_string())?;
